...
/** Usage scenario: look at the documentation for IKeeper class.
  */
class ServiceZooKeeper : public IKeeper
{
public:
    struct Node
    {
    ...
    {
        if (use_ch_service)
            return ser_session_id;
        else
            return zk_session_id;
    }
...
private:
    ...
    Poco::Net::StreamSocket zk_socket;
    std::optional<ReadBufferFromPocoSocket> zk_in;
    std::optional<WriteBufferFromPocoSocket> zk_out;

    Poco::Net::StreamSocket ser_socket;
    std::optional<ReadBufferFromPocoSocket> ser_in;
    std::optional<WriteBufferFromPocoSocket> ser_out;

    int64_t zk_session_id = 0;
    int64_t ser_session_id = 0;

    std::atomic<XID> next_xid {1};
    std::atomic<bool> zk_expired {false};
    std::atomic<bool> ser_expired {false};
    /// Mark session finalization start. Used to avoid simultaneous
    /// finalization from different threads. One-shot flag.
    std::atomic<bool> zk_finalization_started {false};
    std::atomic<bool> ser_finalization_started {false};
    std::mutex zk_push_request_mutex;
    std::mutex ser_push_request_mutex;
    ...
    RequestsQueue zk_requests_queue{1};
    RequestsQueue ser_requests_queue{1};
    void pushRequest(RequestInfo && info);
    bool zkPushRequest(RequestInfo && info);
    bool serPushRequest(RequestInfo && info);

    using Operations = std::map<XID, RequestInfo>;

    Operations zk_operations;
    std::mutex zk_operations_mutex;

    Operations ser_operations;
    std::mutex ser_operations_mutex;

    using SerZkResponsesComp = std::map<XID, ZooKeeperResponsePtr>;

    std::mutex responses_mutex;
    SerZkResponsesComp ser_zk_responses;

    bool use_ch_service = true;

    using WatchCallbacks = std::vector<WatchCallback>;
    using Watches = std::map<String /* path, relative of root_path */, WatchCallbacks>;

    Watches zk_watches;
    std::mutex zk_watches_mutex;

    Watches ser_watches;
    std::mutex ser_watches_mutex;

    ThreadFromGlobalPool zk_send_thread;
    ThreadFromGlobalPool zk_receive_thread;

    ThreadFromGlobalPool ser_send_thread;
    ThreadFromGlobalPool ser_receive_thread;
    ...
    void zkSendHandshake();
    void zkReceiveHandshake();

    void serSendHandshake();
    void serReceiveHandshake();

    void sendAuth(const String & scheme, const String & data);

    void zkReceiveEvent();
    void serReceiveEvent();

    void zkSendThread();
    void zkReceiveThread();

    void serSendThread();
    void serReceiveThread();
    ...
    template <typename T>
    void zkWrite(const T &);

    template <typename T>
    void zkRead(T &);

    template <typename T>
    void serWrite(const T &);
