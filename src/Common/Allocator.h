...
public:
    ...
    void * realloc(void * buf, size_t old_size, size_t new_size, size_t alignment = 0)
    {
        ...
        else if (old_size >= MMAP_THRESHOLD && new_size >= MMAP_THRESHOLD)
        {
            ...
            if (MAP_FAILED == buf)
                RK::throwFromErrno(fmt::format("Allocator: Cannot mremap memory chunk from {} to {}.",
                    ReadableSize(old_size), ReadableSize(new_size)), RK::ErrorCodes::CANNOT_MREMAP);
...
private:
    void * allocNoTrack(size_t size, size_t alignment)
    {
        ...
        if (size >= MMAP_THRESHOLD)
        {
            if (alignment > mmap_min_alignment)
                throw RK::Exception(fmt::format("Too large alignment {}: more than page size when allocating {}.",
                    ReadableSize(alignment), ReadableSize(size)), RK::ErrorCodes::BAD_ARGUMENTS);

            buf = mmap(getMmapHint(), size, PROT_READ | PROT_WRITE,
                       mmap_flags, -1, 0);
            if (MAP_FAILED == buf)
                RK::throwFromErrno(fmt::format("Allocator: Cannot mmap {}.", ReadableSize(size)), RK::ErrorCodes::CANNOT_ALLOCATE_MEMORY);
        ...
        else
        {
            if (alignment <= MALLOC_MIN_ALIGNMENT)
            {
                ...
                if (nullptr == buf)
                    RK::throwFromErrno(fmt::format("Allocator: Cannot malloc {}.", ReadableSize(size)), RK::ErrorCodes::CANNOT_ALLOCATE_MEMORY);
            }
            else
            {
                buf = nullptr;
                ...
                if (0 != res)
                    RK::throwFromErrno(fmt::format("Cannot allocate memory (posix_memalign) {}.", ReadableSize(size)),
                        RK::ErrorCodes::CANNOT_ALLOCATE_MEMORY, res);
    ...
    void freeNoTrack(void * buf, size_t size)
    {
        if (size >= MMAP_THRESHOLD)
        {
            if (0 != munmap(buf, size))
                RK::throwFromErrno(fmt::format("Allocator: Cannot munmap {}.", ReadableSize(size)), RK::ErrorCodes::CANNOT_MUNMAP);
        }
        else
        {
            ::free(buf);
    ...
    void checkSize(size_t size)
    {
        ...
        if (size >= 0x8000000000000000ULL)
            throw RK::Exception(RK::ErrorCodes::LOGICAL_ERROR, "Too large size ({}) passed to allocator. It indicates an error.", size);
    }
