...
public:
    PollSetImpl() : _epollfd(epoll_create(1)), _events(1024), _eventfd(eventfd(0, EFD_NONBLOCK))
    {
    ...
    bool has(const Socket & socket) const
    {
        Poco::FastMutex::ScopedLock lock(_mutex);
        SocketImpl * sockImpl = socket.impl();
        return sockImpl && (_socketMap.find(sockImpl) != _socketMap.end());
    }
    ...
    PollSet::SocketModeMap poll(const Poco::Timespan & timeout)
    {
        ...
        for (int i = 0; i < rc; i++)
        {
            if (_events[i].data.ptr == log)
            {
                ...
                LOG_TRACE(
                    log, "Poll wakeup {} {} {} {}", Poco::Thread::current() ? Poco::Thread::current()->name() : "main", _eventfd, n, errno);
                if (n < 0)
                    errno;
            }
