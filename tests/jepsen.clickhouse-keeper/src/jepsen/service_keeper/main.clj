(ns jepsen.service-keeper.main
  (:require [clojure.tools.logging :refer :all]
            [jepsen.service-keeper.utils :refer :all]
            [clojure.pprint :refer [pprint]]
            [jepsen.service-keeper.db :refer :all]
            [jepsen.service-keeper.set :as set]
            [jepsen.service-keeper.nemesis :as custom-nemesis]
            [jepsen.service-keeper.register :as register]
            [jepsen.service-keeper.unique :as unique]
            [jepsen.service-keeper.queue :as queue]
            [jepsen.service-keeper.counter :as counter]
            [jepsen.service-keeper.constants :refer :all]
...
(def workloads
  ...
  {
    ;
    "set" set / workload "register" register / workload;
...
(defn service-keeper-test
  ...
  (let [quorum (boolean (:quorum opts))
    ...
    (merge tests/noop-test
           opts
           {:name (str "service-keeper-quorum=" quorum "-"  (name (:workload opts)) "-" (name (:nemesis opts)))
            :os debian/os
;            :os ubuntu/os
...
(defn -main
  ...
  (cli/run! (merge (cli/single-test-cmd {:test-fn service-keeper-test
                                         :opt-spec cli-opts})
                   (cli/test-all-cmd {:tests-fn (partial all-tests service-keeper-test)
                                      :opt-spec cli-opts})
                   (cli/serve-cmd))
            args))
