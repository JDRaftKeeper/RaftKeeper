...
node1 = cluster1.add_instance('node', main_configs=['configs/enable_test_keeper_old.xml', 'configs/log_conf.xml'],
                              stay_alive=True, use_old_bin=True)
...
def test_data(started_cluster):
    try:
        ...
        node1.copy_file_to_container(os.path.join(SCRIPT_DIR, "configs/enable_test_keeper.xml"),
                                     '/etc/raftkeeper-server/config.xml')
