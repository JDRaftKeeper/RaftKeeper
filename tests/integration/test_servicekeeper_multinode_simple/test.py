...
def test_simple_replicated_table(started_cluster):
    ...
    node4 = cluster2.add_instance('node4', main_configs=['configs/log_conf.xml', 'configs/use_test_keeper.xml'], stay_alive=True)
    node5 = cluster2.add_instance('node5', main_configs=['configs/log_conf.xml', 'configs/use_test_keeper.xml'], stay_alive=True)
    node6 = cluster2.add_instance('node6', main_configs=['configs/log_conf.xml', 'configs/use_test_keeper.xml'], stay_alive=True)

    try:
        # cluster1.start()
        wait_nodes(cluster1, node1, node2, node3)
        cluster2.start(destroy_dirs=False)

        # cluster2.start()
        for i, node in enumerate([node4, node5, node6]):
        ...
        assert node6.query("SELECT COUNT() FROM t") == "10\n"
